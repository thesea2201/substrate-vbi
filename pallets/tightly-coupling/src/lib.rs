...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight(10_000 + T::DbWeight::get().writes(1))]
		pub fn call_helper_function_from_template(origin: OriginFor<T>, new_value: u32) -> DispatchResult {
			// Check that the extrinsic was signed and get the signer.
			// This function will return an error if the extrinsic is not signed.
			// https://docs.substrate.io/v3/runtime/origins
			let _who = ensure_signed(origin)?;
			...
			// let data = pallet_template::Pallet::<T>::something().unwrap();
			let _ = pallet_template::Pallet::<T>::update_storage(new_value);

			// Emit an event.
			Self::deposit_event(Event::UpdateStore(new_value));
			// Return a successful DispatchResultWithPostInfo
			Ok(())
		}
	}
